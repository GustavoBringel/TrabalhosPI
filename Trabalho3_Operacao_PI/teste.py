...
# =========================================================
NOME_IMAGEM_1 = "imagem_teste1.png"
NOME_IMAGEM_2 = "imagem_teste2.png"
# =========================================================
...
def multiplicacao_imagens(img1, img2):
    ...
    img2_resized = cv2.resize(img2, (img1.shape[1], img1.shape[0]))
    # Converte para float para evitar overflow e normaliza
...
if __name__ == "__main__":
    ...
    # Conversão para o formato RGB (para exibição com matplotlib)
    ...
    # --- Aplicações das Operações Geométricas (na Imagem 1) ---
    ...
    # Aritméticas
    axes[0, 0].imshow(cv2.cvtColor(img_adicao, cv2.COLOR_BGR2RGB))
    axes[0, 0].set_title('Adição')
    axes[0, 0].axis('off')

    axes[0, 1].imshow(cv2.cvtColor(img_subtracao, cv2.COLOR_BGR2RGB))
    axes[0, 1].set_title('Subtração')
    axes[0, 1].axis('off')

    axes[0, 2].imshow(cv2.cvtColor(img_multiplicacao, cv2.COLOR_BGR2RGB))
    axes[0, 2].set_title('Multiplicação')
    axes[0, 2].axis('off')

    # Geométricas
    axes[1, 0].imshow(cv2.cvtColor(img_divisao, cv2.COLOR_BGR2RGB))
    axes[1, 0].set_title('Divisão')
    axes[1, 0].axis('off')
    
    axes[1, 1].imshow(cv2.cvtColor(img_rotacao, cv2.COLOR_BGR2RGB))
    axes[1, 1].set_title('Rotação (45°)')
    axes[1, 1].axis('off')
    
    axes[1, 2].imshow(cv2.cvtColor(img_translacao, cv2.COLOR_BGR2RGB))
    axes[1, 2].set_title('Translação (+50, +30)')
    axes[1, 2].axis('off')
    ...
    axes[2, 2].imshow(img1_rgb)
    axes[2, 2].set_title('Original (Imagem 1)')
    axes[2, 2].axis('off')
